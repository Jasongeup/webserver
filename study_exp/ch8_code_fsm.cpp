...
enum HTTP_CODE{NO_REQUEST, GET_REQUEST, BAD_REQUEST, FORBIDDEN_REQUEST, INTERNAL_ERROR, CLOSED_ERROR};
/* 简化问题，我们没有给客户端发送一个完整的HTTP应答报文，而只是根据服务器的处理结果发送如下成功或失败信息*/
    ...
    for(; checked_index < read_index; ++checked_index){
        ...
        if(temp == '\r'){
            /*如果“\r”字符碰巧是目前buffer中的最后一个已经被读入的客户数据，那么这次分
              析没有读取到一个完整的行，返回LINE_OPEN以表示还需要继续读取客户数据才能进一步分析*/
            if(checked_index + 1 == read_index) return LINE_OPEN;
            // 如果下一个字符是“\n”，则说明我们成功读取到一个完整的行
            else if(buffer[checked_index + 1] == '\n'){
                buffer[checked_index++] = '\0';   // 读取到一个完整的行，然后将\r替换成\0截断这一行
...
HTTP_CODE parse_requestline(char* temp, CHECK_STATE& checkstate){
    ...
    *url++ = '\0';
    char* method = temp;
    // 仅支持GET方法
    if(strcasecmp(method, "GET") == 0) cout << "The request method is GET" << endl;
...
HTTP_CODE parse_headers(char* temp){
    ...
    else cout << "I can't handle this header" << endl;
    return NO_REQUEST;
}
...
HTTP_CODE parse_content(char* buffer, int& checked_index, CHECK_STATE& checkstate, int& read_index, int& start_line){
    ...
    while((linestatus = parse_line(buffer, checked_index, read_index)) == LINE_OK){  // 遍历每一行
        char* temp = buffer + start_line; // start_line是行在buffer中的起始位置
        start_line = checked_index;       // 记录下一行的起始位置，因为parse_line会读取一行，并修改checked_index的位置，使其指向下一行开头
...
int main(int argc, char* argv[]){
    ...
    if(fd < 0) cout << "errno is :" << errno << endl;
    else{
        ...
        while(1){  // 循环读取客户数据并分析
            ...
            if(result == NO_REQUEST) continue;   // 尚未得到一个完整的HTTP请求，需要继续从socket上读取数据, 但是进入下一个while时，读缓冲区已满，最终通知读失败
